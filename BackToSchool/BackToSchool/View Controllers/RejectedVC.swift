...
class RejectedCollectionCell: UICollectionViewCell {
...
class RejectedVC: UIViewController {
    ...
    //MARK: Outlets
    @IBOutlet weak var collection_rejectedwallpapers: UICollectionView!
    
    //MARK: Variables
    var rejectedwallpaperArr = [[String:String]]()
    var pageIndex = 1
    ...
    override func viewDidLoad() {
        ...
        self.collection_rejectedwallpapers.collectionViewLayout = layout
        layout.delegate = self
        layout.numberOfColumns = 2
        
        self.collection_rejectedwallpapers.refreshControl = self.refreshControl
        ...
        let params: NSDictionary = ["pageIndex":"\(self.pageIndex)",
            ...
            "status":"rejected"]
        self.Webservice_RejectedWallpapers(url: urlString, params: params)
    }
    
}

//MARK: Functions
extension RejectedVC {
    ...
    @objc func btnDelete_Clicked(sender: UIButton) {
        ...
        let yesAction = UIAlertAction(title: "네", style: .default) { (action) in
            ...
            let params: NSDictionary = ["user_id":UserDefaults.standard.value(forKey: UD_userId) as! String,
                                        "wallpaper_id":self.rejectedwallpaperArr[sender.tag]["id"]!]
...
extension RejectedVC: UICollectionViewDelegate,UICollectionViewDataSource,PinterestLayoutDelegate {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let rect = CGRect(origin: CGPoint(x: 0,y: 0), size: CGSize(width: self.collection_rejectedwallpapers.bounds.size.width, height: self.collection_rejectedwallpapers.bounds.size.height))
        let noDataImage = UIImageView(frame: rect)
        noDataImage.contentMode = .scaleAspectFit
        noDataImage.image = UIImage(named: "ic_noData")
        self.collection_rejectedwallpapers.backgroundView = noDataImage
        if self.rejectedwallpaperArr.count == 0 {
            noDataImage.isHidden = false
        }
        else {
            noDataImage.isHidden = true
        }
        return self.rejectedwallpaperArr.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = self.collection_rejectedwallpapers.dequeueReusableCell(withReuseIdentifier: "RejectedCollectionCell", for: indexPath) as! RejectedCollectionCell
        cell.backgroundColor = UIColor.init(hex: self.rejectedwallpaperArr[indexPath.item]["wallpaper_color"]!)
        cell.imgWallpaper.isHidden = true
        cell.imgWallpaper.sd_setImage(with: URL(string: self.rejectedwallpaperArr[indexPath.item]["wallpaper_image"]!)) { (image, error, cache, url) in
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if indexPath.item == self.rejectedwallpaperArr.count - 1 {
            if self.pageIndex != 0 {
                let urlString = API_URL + "getmywallpapers.php"
                let params: NSDictionary = ["pageIndex":"\(self.pageIndex)",
                    "numberOfRecords":numberOfRecords,
    ...
    func collectionView(collectionView: UICollectionView,
        ...
        let wallpaperHeight = Int(self.rejectedwallpaperArr[indexPath.item]["wallpaper_height"]!)
...
extension RejectedVC
{
    func Webservice_RejectedWallpapers(url:String, params:NSDictionary) -> Void {
        WebServices().CallGlobalAPI(url: url, headers: [:], parameters:params, httpMethod: "POST", progressView:true, uiView:self.view, networkAlert: true) {(_ jsonResponse:JSON? , _ strErrorMessage:String) in
            ...
            if strErrorMessage.count != 0 {
                ...
                if responseCode == "1" {
                    ...
                    if self.pageIndex == 1 {
                        self.rejectedwallpaperArr.removeAll()
                    }
                    ...
                    for data in responseData {
                        ...
                        self.rejectedwallpaperArr.append(wallpaperObj)
                    }
                    self.collection_rejectedwallpapers.reloadData()
                }
    ...
    func Webservice_DeleteWallpaper(url:String, params:NSDictionary, wallpaperIndex:Int) -> Void {
        WebServices().CallGlobalAPI(url: url, headers: [:], parameters:params, httpMethod: "POST", progressView:true, uiView:self.view, networkAlert: true) {(_ jsonResponse:JSON? , _ strErrorMessage:String) in
            ...
            if strErrorMessage.count != 0 {
                ...
                let responseCode = jsonResponse!["ResponseCode"].stringValue
                if responseCode == "1" {
                    self.rejectedwallpaperArr.remove(at: wallpaperIndex)
                    self.collection_rejectedwallpapers.reloadData()
                }
                else {
